  ...
  switch (ch) {
    ...
    case KEY_UP:
      if (ui->focus == FOCUS_QUERY) {
        ...
        if (crow > 0 && ui->history_idx == ui->history_count) {
          ...
          int fw = cols_now - INPUT_PROMPT_W;
          if (fw < 1) fw = 1;
          ...
          if (ui->history_count > 0 && ui->history_idx > 0) {
            ...
            if (hlen >= ui->input_cap) {
              int new_cap = hlen + 64;
              if (safe_realloc((void **)&ui->input_buffer, (size_t)new_cap)) {
                ui->history_idx++;
                break;
              }
              ui->input_cap = new_cap;
    ...
    case KEY_RIGHT:
      if (ui->focus == FOCUS_QUERY)
        ...
        /* Only allow horizontal scroll when content is wider than pane */
        int max_tw = ui->doc ? ui->doc->max_content_width : 0;
