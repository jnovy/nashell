  ...
  while (n_segs > 0) {
    ...
    if (fit_count > 0 || partial_seg >= 0) {
      ...
      if (n_segs > 0 && segs[0].len > 0) {
        ...
        int b = 0;
        while (b < segs[0].len && segs[0].text[b] && (segs[0].text[b] & 0xC0) == 0x80)
          b++;
        if (b < segs[0].len) {
...
int md_render(WINDOW *win, md_doc_t *doc, int scroll_y, int scroll_x,
  ...
  while (*src) {
    ...
    } else if (line_buf[0] == '~' && line_buf[1] == '>' && line_buf[2] == ' ') {
...
int md_find_anchor(md_doc_t *doc, const char *fragment, int cols) {
  ...
  while (*src) {
    ...
    if (!in_code_fence && src[0] == '#') {
      ...
      int level = 0;
      while (level < line_len && src[level] == '#')
        level++;
      int start = level;
      while (start < line_len && src[start] == ' ')
        start++;
