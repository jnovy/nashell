...
static const char *cause_str(sh_cause_t c) {
  switch (c) {
    case SH_CAUSE_NO_DONE:
      return "no_done";
    case SH_CAUSE_WRONG_RESULT:
      return "wrong_result";
    case SH_CAUSE_WRONG_TOOL:
      return "wrong_tool";
    case SH_CAUSE_TOO_MANY_STEPS:
      return "too_many_steps";
    case SH_CAUSE_ERRORS:
      return "errors";
    case SH_CAUSE_OTHER:
      return "other";
  }
...
static const char *mechanism_str(sh_mechanism_t m) {
  switch (m) {
    case SH_MECH_TOOL_CHOICE:
      return "wrong_tool_choice";
    case SH_MECH_TASK_ABANDON:
      return "task_abandonment";
    case SH_MECH_LOOP:
      return "unproductive_loop";
    case SH_MECH_INCOMPLETE:
      return "incomplete_result";
    case SH_MECH_ERROR_CASCADE:
      return "error_cascade";
    case SH_MECH_OVER_EXPLORE:
...
static const char *status_str(sh_status_t s) {
  switch (s) {
    case SH_STATUS_CAUSAL:
      return "causal";
    case SH_STATUS_CONTRIBUTING:
      return "contributing";
    case SH_STATUS_UNKNOWN:
      return "unknown";
  }
  return "unknown";
}
...
static sh_signature_t classify_failure(const query_result_t *qr) {
  ...
  for (int i = 0; i < qr->n_crit_results; i++) {
    ...
    if (strstr(desc, "status("))
      has_status_fail = 1;
    else if (strstr(desc, "contains("))
      has_contains_fail = 1;
    else if (strstr(desc, "not_contains("))
      has_contains_fail = 1;
    else if (strstr(desc, "tool_used("))
      has_tool_fail = 1;
    else if (strstr(desc, "tool_not_used("))
      has_tool_fail = 1;
    else if (strstr(desc, "max_steps("))
      has_steps_fail = 1;
    else if (strstr(desc, "no_error"))
      has_error_fail = 1;
    else if (strstr(desc, "regex("))
      has_contains_fail = 1;
  }
  ...
  for (int i = 0; i < report->n_bank_results; i++) {
    ...
    for (int j = 0; j < br->n_results; j++) {
      ...
      for (int k = 0; k < bundle->n_clusters; k++) {
        if (sig_equals(&bundle->clusters[k].sig, &sig)) {
          ci = k;
          break;
        }
      }
  ...
  for (int epoch = 0; epoch < n_epochs && !stop_early; epoch++) {
    ...
    for (int round = 1; round <= opt->max_rounds; round++) {
          ...
          /* SkillOpt: generate brief rejection audit with per-query detail */
          {
            ...
            if (reject_reason)
              ...
              str_appendf(&audit, "Regressed on held-in (%.1f%%) "
                                  "despite held-out improvement",
                          d_in * 100);
            else if (d_ho < -0.001)
              str_appendf(&audit, "Regressed on held-out (%.1f%%) "
                                  "despite held-in improvement",
                          d_ho * 100);
            else
      ...
      evidence = round_baseline_report
                   ? optimize_build_evidence_bundle(round_baseline_report)
                   : NULL;
      if (best_report) regression_free_report(best_report);
...
char *optimize_format_manifest_feedback(const manifest_entry_t *manifest) {
  ...
  if (total_fix_pred > 0)
    str_appendf(&fb, "  Fix predictions: %d/%d correct (%.0f%% precision)\n",
                ...
                total_fix_pred > 0
                  ? 100.0 * manifest->correct_fixes / total_fix_pred
                  : 0.0);
  ...
  if (total_risk_pred > 0)
    str_appendf(&fb, "  Risk predictions: %d/%d materialized (%.0f%% precision)\n",
                ...
                total_risk_pred > 0
                  ? 100.0 * manifest->correct_risks / total_risk_pred
                  : 0.0);
