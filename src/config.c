...
void config_set_defaults(config_t *cfg) {
  ...
  if (cfg->dedup_pre_check_threshold <= 0) cfg->dedup_pre_check_threshold = 0.85f;
  ...
  if (cfg->memory_halflife < 0) cfg->memory_halflife = 90.0f;
...
config_t *config_load(const char *path) {
  ...
  if (limits) {
    ...
    cfg->max_reflection_steps = toml_int(limits, "max_reflection_steps", -1);
    {
      ...
      if (rg) {
        if (strcmp(rg, "always") == 0)
          cfg->reflection_gate = 1;
        else if (strcmp(rg, "never") == 0)
          cfg->reflection_gate = 2;
        else
          cfg->reflection_gate = 0; /* "user_ask" or unrecognized */
        free(rg);
      }
    }
    ...
    cfg->dedup_pre_check_threshold = (float)toml_dbl(limits, "dedup_pre_check_threshold", 0);
    ...
    {
      double v = toml_dbl(limits, "memory_halflife", -1);
      if (v >= 0) cfg->memory_halflife = (float)v;
      /* Backward compat: accept old vscore_halflife as alias */
      else {
        v = toml_dbl(limits, "vscore_halflife", -1);
        if (v >= 0) cfg->memory_halflife = (float)v;
      }
    }
    {
  ...
  if (dc) {
    ...
    if (disp) {
      if (strcmp(disp, "webcam") == 0)
        cfg->device_control.display_type = 1;
      else if (strcmp(disp, "command") == 0)
        cfg->device_control.display_type = 2;
      else
        cfg->device_control.display_type = 0; /* vnc (default) */
      free(disp);
    }
    ...
    if (inp) {
      if (strcmp(inp, "hid") == 0)
        cfg->device_control.input_type = 1;
      else if (strcmp(inp, "command") == 0)
        cfg->device_control.input_type = 2;
      else
        cfg->device_control.input_type = 0; /* vnc (default) */
      free(inp);
    }
...
int config_load_model_profiles(config_t *cfg, const char *models_dir) {
  ...
  while ((ent = readdir(d)) != NULL) {
    ...
    if (mem_tbl) {
      ...
      {
        double v = toml_dbl(mem_tbl, "memory_halflife", -2);
        if (v > -2) p->memory_halflife = (float)v;
        /* Backward compat: accept old vscore_halflife as alias */
        else {
          v = toml_dbl(mem_tbl, "vscore_halflife", -2);
          if (v > -2) p->memory_halflife = (float)v;
        }
      }
    ...
    if (tools_tbl) {
      ...
      if (ntabs > 0) {
        ...
        int nkeys = toml_table_nkval(tools_tbl) + toml_table_narr(tools_tbl) + toml_table_ntab(tools_tbl);
        ...
        if (nd == 0) {
          ...
          p->tool_desc_names = NULL;
          free(p->tool_desc_values);
          p->tool_desc_values = NULL;
        }
      }
    }
...
void config_apply_profile(config_t *cfg, const model_profile_t *p) {
  ...
  if (p->memory_halflife > -2.0f) cfg->memory_halflife = p->memory_halflife;
...
void config_dump_spec(const config_t *cfg, FILE *out, const char *profile_file) {
  ...
  fprintf(out, "reflection_gate = \"%s\"\n",
          cfg->reflection_gate == 2 ? "never" : cfg->reflection_gate == 1 ? "always"
                                                                          : "user_ask");
  ...
  fprintf(out, "memory_halflife = %.1f\n", cfg->memory_halflife);
  ...
  fprintf(out, "dedup_pre_check_threshold = %.2f\n", cfg->dedup_pre_check_threshold);
...
int config_load_spec_overlay(config_t *cfg, const char *path) {
     ...
     * Typos like [memeory] or [recat] silently have no effect. */
  {
    ...
    int n_total = toml_table_nkval(root) + toml_table_narr(root) + toml_table_ntab(root);
  ...
  if (mem) {
    ...
    {
      double vs = toml_dbl(mem, "memory_halflife", -1);
      if (vs >= 0) cfg->memory_halflife = (float)vs;
      /* Backward compat: accept old vscore_halflife as alias */
      else {
        vs = toml_dbl(mem, "vscore_halflife", -1);
        if (vs >= 0) cfg->memory_halflife = (float)vs;
      }
    }
    {
  ...
  if (tools) {
    ...
    if (ntabs > 0) {
      ...
      int nd = 0;
      int nkeys = toml_table_nkval(tools) + toml_table_narr(tools) + toml_table_ntab(tools);
      for (int j = 0; j < nkeys; j++) {
  ...
  if (limits) {
    ...
    {
      double d = toml_dbl(limits, "dedup_pre_check_threshold", 0);
      if (d > 0) cfg->dedup_pre_check_threshold = (float)d;
    }
...
int config_write_default(const char *path) {
  ...
  fputs(
    ...
    "dedup_pre_check_threshold = 0.85 # pre-store gate: block memory_store if similar entry exists\n"
    ...
    "memory_halflife = 90.0       # unified temporal decay half-life in days (0 = disabled)\n"
