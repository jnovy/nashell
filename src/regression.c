  ...
  switch (crit->type) {
    case CRIT_STATUS:
      if (crit->expect && strcmp(crit->expect, "done") == 0)
        ...
        cr.passed = !have_result;
      if (!cr.passed)
        cr.detail = xstrdup(have_result ? "got done, expected fail" : "got fail, expected done");
      break;
...
regression_report_t *regression_run(query_bank_t *banks, int n_banks,
  ...
  report->overall_score = total_weight > 0
                            ? (hi_total + ho_total) / total_weight
                            : 0;
        ...
        for (int k = 0; k < qr->n_crit_results; k++) {
          if (!qr->crit_results[k].passed) {
            fprintf(stderr, "      ✗ %s: %s\n",
                    ...
                    qr->crit_results[k].detail ? qr->crit_results[k].detail : "failed");
          }
        }
      }
    }
...
validate_result_t regression_compare(const regression_report_t *baseline,
  ...
  for (int i = 0; i < current->n_bank_results; i++) {
    ...
    for (int j = 0; j < cb->n_results; j++) {
      ...
      if (bq) {
        const char *arrow;
        if (cq->passed && !bq->passed)
          arrow = "✗→✓";
        else if (!cq->passed && bq->passed)
          arrow = "✓→✗";
        else if (cq->passed)
          arrow = "✓→✓";
        else
  ...
  const char *label;
  switch (result) {
    case VALIDATE_ACCEPTED:
      label = "✓ ACCEPTED — harness change improves performance";
      break;
    case VALIDATE_REJECTED:
      label = "✗ REJECTED — harness change degrades performance";
      break;
    case VALIDATE_INCONCLUSIVE:
      label = "? INCONCLUSIVE — no significant change";
      break;
  }
