  ...
  if (model) {
    for (int i = 0; embed_model_limits[i].pattern; i++) {
      if (strstr(model, embed_model_limits[i].pattern)) {
        return embed_model_limits[i].max_tokens * EMBED_DEFAULT_CHARS_PER_TOKEN;
      }
    }
  }
...
embed_vec_t *embed_text_batch(embed_ctx_t *ctx, const char **texts,
  ...
  *out_count = 0;
  ...
  if ((ctx->cfg.type == EMBED_OLLAMA || ctx->cfg.type == EMBED_OPENAI) && ctx->available) {
...
int embed_multi_vec_save(const embed_multi_vec_t *mv, const char *path) {
  ...
  if (mv->n_chunks == 1) {
    ...
    if (fwrite(mv->data, sizeof(float), (size_t)mv->dim, f) != (size_t)mv->dim) {
      fclose(f);
      return -1;
    }
  } else {
    ...
    int32_t dim = (int32_t)mv->dim;
    if (fwrite(&neg_chunks, sizeof(neg_chunks), 1, f) != 1) {
      fclose(f);
      return -1;
    }
    if (fwrite(&dim, sizeof(dim), 1, f) != 1) {
