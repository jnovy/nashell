...
int evict_finalize(react_ctx_t *ctx, llm_chat_t *chat,
  ...
  if (breadcrumb_str) {
    ...
    if (ctx->flags.inject_memory && (ctx->tools->memory || ctx->tools->ws) && strlen(breadcrumb_str) > 100) {
      int ev_candidates = ctx->tools->cfg
                            ? ctx->tools->cfg->eviction_recall_candidates
                            : 3;
      double ev_min_rel = ctx->tools->cfg
                            ? ctx->tools->cfg->eviction_recall_min_relevance
                            : 0.30;
  ...
  if (usage_pct > target_pct) {
    nash_log("[eviction] post-finalize usage %d%% > target %d%% — "
             "stripping scratchpad",
             usage_pct, target_pct);
  ...
  if (usage_pct > target_pct) {
    nash_log("[eviction] WARNING: post-finalize still %d%% > target %d%% — "
             ...
             "falling back to emergency eviction",
             usage_pct, target_pct);
...
int evict_score_progressive_semantic(const llm_chat_t *chat, int mi, int ri,
  ...
  int base_score = imp * REACT_SCORE_IMP_WEIGHT - rec * REACT_SCORE_REC_WEIGHT - size_bonus + pos_norm;
...
static char *evict_build_breadcrumbs(react_ctx_t *ctx, const llm_chat_t *chat,
  ...
  int max_per_msg = n_to_evict > 0
                      ? (int)(breadcrumb_summary_cap / n_to_evict)
                      : pol.summary_per_msg_min;
    ...
    if (chat->msgs[mi].store_alias) {
      ...
      if (n_seen < REACT_BREADCRUMB_MAX_ALIASES) {
        for (int s = 0; s < n_seen; s++) {
          if (strcmp(seen_aliases[s], alias) == 0) {
            dup = 1;
            break;
          }
        }
      ...
      if (chat->msgs[mi].tool_name && strcmp(chat->msgs[mi].tool_name, "file_read") == 0 && chat->msgs[mi].tool_path) {
...
void react_maybe_evict(react_ctx_t *ctx, llm_chat_t *chat, int step,
  ...
  long sp_abs_cap = (context_budget > 0)
                      ? context_budget * pol.sp_budget_pct / 100
                      : pol.sp_fallback;
  ...
  if (emb && emb->available) {
    ...
    /* Find user query */
    for (int i = 0; i < chat->n_msgs; i++) {
      if (chat->msgs[i].msg_type == LLM_MSG_USER_QUERY && chat->msgs[i].content) {
    ...
    for (int i = chat->n_msgs - 1; i >= 0; i--) {
      if (chat->msgs[i].role && strcmp(chat->msgs[i].role, "assistant") == 0 && chat->msgs[i].content && chat->msgs[i].content[0]) {
