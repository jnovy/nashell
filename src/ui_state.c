...
void ui_state_reload_file(ui_state_t *ui) {
  ...
  /* Non-.md files are static store refs.  Wrap their raw content in
   * a markdown heading so they render properly (same as ui_state_enter
   * does for interactive navigation). */
  int len = (int)strlen(ui->current_filepath);
  if (len < 3 || strcmp(ui->current_filepath + len - 3, ".md") != 0) {
    char *raw = slurp_file(ui->current_filepath, NULL);
    if (!raw) return; /* file vanished - keep current doc */
    const char *fname = strrchr(ui->current_filepath, '/');
    fname = fname ? fname + 1 : ui->current_filepath;
