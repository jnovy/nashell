...
static provider_t *create_role_provider(const config_t *cfg,
  ...
  provider_config_t pcfg = {
    ...
    .reasoning_effort = np->config.reasoning_effort,
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--api") == 0 && i + 1 < argc) {
      ...
      if (strcmp(sp, "held-in") == 0)
        regression_split = 0;
      else if (strcmp(sp, "held-out") == 0)
        regression_split = 1;
  ...
  provider_config_t pcfg = {
    ...
    .reasoning_effort = cfg->provider.reasoning_effort,
    .max_tokens = cfg->max_tokens,
    .temperature = cfg->temperature,
    .top_p = cfg->top_p,
    .top_k = cfg->top_k,
  ...
  if (cfg->routing.worker && cfg->routing.default_provider &&
    ...
    if (worker_provider) {
      ...
      server_model = worker_provider->cfg.model_id
                       ? xstrdup(worker_provider->cfg.model_id)
                       : NULL;
    }
  }
