...
static const char *format_recency(double created_at, char *buf, size_t bufsz) {
  ...
  if (days == 0)
    snprintf(buf, bufsz, "today");
  else if (days == 1)
    snprintf(buf, bufsz, "1d ago");
  else if (days < 7)
    snprintf(buf, bufsz, "%dd ago", days);
  else if (days < 30)
    snprintf(buf, bufsz, "%dw ago", days / 7);
  else if (days < 365)
    snprintf(buf, bufsz, "%dmo ago", days / 30);
  else
    snprintf(buf, bufsz, "%dy ago", days / 365);
  return buf;
}
...
void react_inject_recall_context(llm_chat_t *chat, react_ctx_t *ctx,
         ...
         * of recent memory activity to enable temporal reasoning. */
  {
    int do_temporal = ctx->tools->cfg
                        ? ctx->tools->cfg->temporal_calendar
                        : 1;
    if (do_temporal) {
      int recent_days = ctx->tools->cfg
                          ? ctx->tools->cfg->temporal_recent_days
                          : 7;
      int older_days = ctx->tools->cfg
                         ? ctx->tools->cfg->temporal_older_days
                         : 30;
      int max_entries = ctx->tools->cfg
                          ? ctx->tools->cfg->temporal_max_entries
                          : 20;
      double now = (double)time(NULL);
      ...
      if (emb_ctx && emb_ctx->available) {
        ...
        if (query_emb.data) {
          int ep_max = ctx->tools->cfg
                         ? ctx->tools->cfg->episodic_max_results
                         : 2;
          double ep_min = ctx->tools->cfg
                            ? ctx->tools->cfg->episodic_min_score
                            : 0.35;
         ...
         * associative recall. */
  {
    int assoc_depth = ctx->tools->cfg
                        ? ctx->tools->cfg->associative_depth
                        : 1;
    if (assoc_depth > 0) {
